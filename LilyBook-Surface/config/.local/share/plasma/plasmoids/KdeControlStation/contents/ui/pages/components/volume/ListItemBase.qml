...
PC3.ItemDelegate {
    ...
    opacity: (volumePage.draggedStream && volumePage.draggedStream.deviceIndex === item.model.Index) ? 0.3 : 1.0
    ...
    contentItem: RowLayout {
        ...
        Kirigami.Icon {
            ...
            Drag.onDragStarted: {
                volumePage.draggedStream = item.model.PulseObject;
                beginMoveStream(item.type === "sink-input" ? "sink" : "source");
            }
            Drag.onDragFinished: {
                volumePage.draggedStream = null;
                endMoveStream();
            }
        }
    ...
    DropArea {
        ...
        parent: item
        anchors.fill: parent
        enabled: volumePage.draggedStream && volumePage.draggedStream.deviceIndex !== item.model.Index
        onDropped: drop => {
            volumePage.draggedStream.deviceIndex = item.model.Index;
        }
    }
    ...
    ListItemMenu {
        ...
        cardModel: volumePage.paCardModel
        itemType: {
            switch (item.type) {
            case "sink":
        ...
        sourceModel: if (item.type.startsWith("sink")) {
            return volumePage.paSinkFilterModel
        }  else if (item.type.startsWith("source")) {
            return volumePage.paSourceFilterModel
        }
    }
